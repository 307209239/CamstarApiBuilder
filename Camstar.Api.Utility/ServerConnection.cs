...
namespace Camstar.Api.Utility
{
    public class ServerConnection
    {
        ...
        private ILogger _EventLog = null;
        ...
        public ServerConnection()
        {
            ...
            if (_EventLog == null)
                _EventLog = LogManager.GetLogger("InSite Server Connection Library");
        }

        public virtual string Host { get; set; } = "localhost";


        public virtual int Port { get; set; } = 2881;

        public virtual int SendTimeout { get; set; } = 90000;

        public virtual int ReceiveTimeout { get; set; } = 90000;

        public virtual string XMLLogPath { get; set; } = "C:\\Temp";

        public virtual bool LogXML { get; set; }
        public ApplicationProtocol Protocol { get; set; }
        public virtual string InboundXML { get; set; }

        public virtual string InboundXMLFile
        {
            get { return this._InboundXMLFile; }
            set
            {
        ...
        public string AppServerUriStem { get; set; }="/camstarappserver/api/";
        public bool IgnoreInvalidCert { get; set; } = true;
        ...
        public virtual bool Connect()
        {
            ...
            if (this.Protocol == ApplicationProtocol.Auto)
            {
                switch (this.Port)
                {
                    case 80:
                        this.Protocol = ApplicationProtocol.Http;
                        break;
                    case 443:
                        this.Protocol = ApplicationProtocol.Https;
                        break;
                    case 2881:
                        this.Protocol = ApplicationProtocol.InSiteSockets;
                        break;
                    case 8080:
                        this.Protocol = ApplicationProtocol.Http;
                        break;
            ...
            if (!this.Protocol.Equals((object) ApplicationProtocol.InSiteSockets))
                return true;
            try
            {
                this._TcpClient = new TcpClient();
                this._TcpClient.ReceiveBufferSize = 75000;
                this._TcpClient.SendTimeout = this.SendTimeout;
                this._TcpClient.ReceiveTimeout = this.ReceiveTimeout;
            ...
            catch (SocketException ex)
            {
                ...
                this.LogError($"Connect to {(object) this.Host}:{(object) this.Port}", (Exception) ex);
                throw ex;
            }
        }
        ...
        {
            return this.Submit(host, port, (string) null, (string) null, this.LogXML, (string) null);
        }

        public virtual string Submit(string host, int port, string inboundXML)
        {
            return this.Submit(host, port, inboundXML, (string) null, this.LogXML, (string) null);
        }

        public virtual string Submit(string host, int port, string inboundXML, bool logXML)
        {
            return this.Submit(host, port, inboundXML, (string) null, logXML, (string) null);
        }
        ...
        {
            return this.Submit(host, port, inboundXML, inboundXMLFilePath, this.LogXML, (string) null);
        }
            ...
            string xmlLogPath)
        {
            ...
            if (!StringUtil.IsEmptyString(xmlLogPath))
                this.XMLLogPath = xmlLogPath;
            return this.Submit();
        }

        public virtual string Submit(string host)
        {
            return this.Submit(host, -1, (string) null, (string) null, this.LogXML, (string) null);
        }

        public virtual string Submit(string host, string inboundXML)
        {
            return this.Submit(host, -1, inboundXML, (string) null, this.LogXML, (string) null);
        }

        public virtual string Submit(string host, string inboundXML, bool logXML)
        {
            return this.Submit(host, -1, inboundXML, (string) null, logXML, (string) null);
        }

        public virtual string Submit(string host, string inboundXML, bool logXML, string xmlLogPath)
        {
            return this.Submit(host, -1, inboundXML, (string) null, logXML, xmlLogPath);
        }

        public virtual string Submit(string host, string inboundXML, string inboundXMLFilePath)
        {
            return this.Submit(host, -1, inboundXML, inboundXMLFilePath, this.LogXML, (string) null);
        }
        ...
        protected virtual string SendReceivetHttp()
        {
            ...
            bool flag = ApplicationProtocol.Https == this.Protocol;
            UriBuilder uriBuilder = new UriBuilder();
            uriBuilder.Host = this.Host;
            uriBuilder.Path = this.AppServerUriStem;
            ...
            if (flag && 443 != this.Port || !flag && 80 != this.Port)
            ...
            if (-1 != this.InboundXML.IndexOf("<__service"))
            {
                ...
                string str2 =
                    this.InboundXML.Substring(startIndex, this.InboundXML.IndexOf("\">", startIndex) - startIndex);
        ...
        {
            this._EventLog.Error($"{(object) message}\r\n{(object) ex.ToString()}");
        }

        protected void LogWarning(string message, Exception ex)
        {
            this._EventLog.Warn($"{(object) message}\r\n{(object) ex.ToString()}");
        }

        protected void LogInfo(string message)
        {
            this._EventLog.Info(message);
        }
        ...
        protected virtual string GetLogFile(string suffix)
        {
            ...
            stringBuilder.Append(this.XMLLogPath);
            if (!this.XMLLogPath.EndsWith("\\"))
                stringBuilder.Append("\\");
            stringBuilder.Append(DateTime.Now.ToString("yyyyMMddHHmmssfff"));
            try
            {
                stringBuilder.Append(Thread.CurrentThread.ManagedThreadId.ToString("d2"));
            }
            catch (Exception ex)
            {
        ...
        protected virtual string EncryptPassword(string document)
        {
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    do
                    {
                        ...
                        {
                            for (bool flag2 = reader.MoveToFirstAttribute();
                                flag2;
            ...
            {
                str1 = document;
                this._EventLog.Error($"Password encrypting error: {ex.Message}");
            }
            finally
            {
                reader?.Close();
        ...
        protected virtual void LogDocument(string operation, string document)
        {
            try
            {
                if (!this.CheckLogPath(this.XMLLogPath))
                    return;
                string document1 = document.Replace("encoding=\"utf-16\"", "").Replace("encoding='UTF-16LE'", "")
        ...
        {
            return this.IgnoreInvalidCert || this.IsHostLocalMachine(this.Host) || errors == SslPolicyErrors.None;
        }
        ...
        private bool IsHostLocalMachine(string hostName)
        {
            if (!string.IsNullOrWhiteSpace(hostName))
            {
                ...
                if (hostName.Equals("localhost", StringComparison.InvariantCultureIgnoreCase) ||
                    hostName.Equals("127.0.0.1", StringComparison.InvariantCultureIgnoreCase) ||
                    (hostName.Equals("::1", StringComparison.InvariantCultureIgnoreCase) ||
                     hostName.Equals(machineName, StringComparison.InvariantCultureIgnoreCase)))
                    return true;
                IPHostEntry hostEntry = Dns.GetHostEntry(machineName);
                foreach (string aliase in hostEntry.Aliases)
                {
                    if (hostName.Equals(aliase, StringComparison.InvariantCultureIgnoreCase))
                        return true;
                }
