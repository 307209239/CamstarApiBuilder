  ...
  public class DateUtil
  {
    ...
    public static string StripTimeZoneOffset(string dateTimeWithOffset)
    {
      if (dateTimeWithOffset == null)
        throw new ArgumentNullException(nameof (dateTimeWithOffset));
      string[] strArray1 = dateTimeWithOffset.Split(DateUtil.TimeSeparator);
      if (strArray1.Length != 2)
        throw new ArgumentException("Argument must contain 'T' in order to separate date and time.", nameof (dateTimeWithOffset));
    ...
    public static string DateTimeFormat
    {
      get
      {
        return string.Format("{0} {1}", (object) DateUtil.DateFormat, (object) DateUtil.TimeFormat);
      }
    }

    public static string DateTimeFormatPHP
    {
      get
      {
        return string.Format("{0} {1}", (object) DateUtil.DateFormat.ToLower().Replace("yyyy", "Y").Replace("yy", "y").Replace("d", "j").Replace("jj", "d").Replace("m", "n").Replace("nn", "m"), (object) DateUtil.TimeFormat.ToLower().Replace("hh", "H").Replace("mm", "i").Replace("ss", "s").Replace("tt", "A").Replace("fff", "u"));
      }
    }
